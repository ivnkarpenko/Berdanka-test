...
`X=0;Y=0` означает нулевой базис, который устройство запомнило после
15-секундного прогрева IMU. Повторная отправка одного и того же `MSG` не
сдвигает цель дальше. Положительный `Y` смещает маркер вправо, положительный
`X` смещает маркер вверх. Дальность в Arduino сейчас не используется и не
влияет на положение квадрата без UWB/позиции устройства.
