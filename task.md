...
- `X` - смещение угла места от нулевого pitch-базиса после 15-секундной калибровки, градусы;
- `Y` - смещение азимута от нулевого yaw-базиса после 15-секундной калибровки, градусы;
...
`X=0;Y=0` центрирует цель в стартовом нулевом базисе устройства. Это основная
...
- Jetson-кнопка Center Arduino должна отправлять `MSG` с `X=0;Y=0`, чтобы
  центрировать цель в стартовом нулевом базисе устройства.
...
- `MSG` с `X=0;Y=0` должен центрировать цель в стартовом нулевом базисе устройства.
