# Berdanka-test: техническое задание и текущее состояние

## 1. Назначение

`Berdanka-test` - аппаратный тестовый проект для переносимого устройства с
дисплеем. Он должен показать на TFT, куда относительно текущей ориентации
устройства находится цель, полученная от внешнего источника целеуказания.

Проект является практической проверкой части математики из
`berdanka-vizualize`, но без UWB-позиционирования на текущем этапе.

## 2. Аппаратная платформа

- MCU: `Arduino UNO R4 WiFi`.
- IMU: `ICM-20948`, подключение по I2C.
- Дисплей: `ILI9488`, SPI, рабочая ориентация landscape `480x320`.
- Wi-Fi: встроенный модуль UNO R4 WiFi.

Пины дисплея в прошивке:

```text
TFT_CS  = 10
TFT_DC  = 9
TFT_RST = 8
TFT_LED = A0
```

## 3. Что сейчас реализовано

- Arduino поднимает Wi-Fi AP `cisco` / `cisco1234`.
- TCP-сервер слушает порт `3333`.
- IMU читается через DMP `GAME_ROTATION_VECTOR` / Quat6, без магнитометра.
  На старте прошивка постоянно вычитывает FIFO и берет zero сразу после
  короткого непрерывного стабильного окна IMU. Если устройство движется прямо в
  момент zero, на экране остается `KEEP STILL`, а прошивка ждет новое стабильное
  окно. Направление цели остается относительным: при старте и `CMD:CENTER` yaw
  обнуляется в текущий базис, абсолютный север не используется.
- Монтаж IMU в Arduino-версии: `Y` идет прямо сквозь дисплей, `X` идет вправо
  по дисплею, `Z` идет вверх по дисплею. Курс/yaw считается вокруг `Z`;
  вертикальное смещение маркера берется из roll-компонента устройства вокруг
  `X`.
- После zero-калибровки Quat6 yaw продолжает жить без магнитного ориентира,
  поэтому прошивка включает runtime auto-zero: когда устройство неподвижно,
  `zeroYaw` медленно подтягивается к raw yaw и убирает уход маркера. В IMU-логе
  для диагностики есть `zero_ready`, `zero_stable_ms`, `yaw_auto_zero`,
  `yaw_still_ms` и `yaw_auto_zero_deg`.
- При старте показывается черный экран с прицелом, маркером в центре и
  коротким zero-индикатором в углу; прошивка сразу вычитывает DMP FIFO, быстро
  выполняет нулевую калибровку и заново центрирует маркер.
- На экране остаются только статусы IMU и сети (`NET WAIT`, `TCP OK`, `UDP OK`);
  числовой HUD не рисуется.
- Маркер цели рисуется поверх прицела.
- Маркер зеленый, если устройство наведено в допуске, иначе красный.
- При `NET WAIT` маркер полый, при `TCP OK` или `UDP OK` маркер залитый.
- Есть throttling TCP и рендера маркера для снижения лагов; UDP читается без
  установления соединения.
- Настройки экрана и рендера можно менять одинаковыми командами по TCP или UDP.
- EEPROM не используется: цель и `Msg` не сохраняются между перезапусками.
- Обратная телеметрия Arduino -> Jetson не используется.
- Добавлена легкая Jetson Linux control panel для задания цели, настроек и круговой проверки отрисовки.
- TCP GUI для Jetson и Windows синхронизированы: в обеих версиях есть ручная
  цель, YOLO-захват, контрастный OpenCV-трекер и ORB OpenCV-трекер. Contrast и
  ORB выводят на изображение камеры только одну выбранную точку, а диагностика
  и пояснения к настройкам показаны в GUI.

## 4. Что сознательно не используется

### UWB

UWB пока не используется в `Berdanka-test`.

Сейчас прошивка не принимает позицию устройства, расстояния до якорей и не
делает компенсацию смещения переносимого устройства. Это соответствует режиму
`no_uwb` из `berdanka-vizualize`: устройство получает направление на цель и
отображает его относительно своей IMU-ориентации.

UWB можно добавить позже отдельным этапом:

- принять позицию устройства в мировой системе координат;
- рассчитать вектор `target - device_position`;
- использовать ту же проекцию на экран;
- для реальной 2D-схемы желательно 3 якоря, для 3D - 4 якоря.

### DeepStream и TensorRT engine

DeepStream-конфиги, custom parser и TensorRT engine-сценарий удалены из проекта.
Для Jetson остается обычный Python GUI. Если нужна детекция, она должна идти
через локальную YOLO/ONNX модель в GUI без DeepStream pipeline.

## 5. Согласование с berdanka-vizualize

Визуализатор задает мировую систему:

```text
X - восток / вправо
Y - север / вперед
Z - вверх
```

Цель задается из статического источника как:

```text
azimuth_deg
elevation_deg
range_m
```

В текущей прошивке `range_m` не используется, потому что без UWB и без знания
позиции переносимого устройства дальность не меняет направление на экране.
В Jetson control panel предполагаемая дальность цели по умолчанию
задана `20 м`.

Главное правило, перенесенное из `berdanka-vizualize`: экран показывает не
`360°`, а настраиваемое поле зрения.

```text
deg_per_px_x = FOV_X / SCREEN_W
deg_per_px_y = FOV_Y / SCREEN_H
```

В прошивке используется обратная форма:

```text
px_per_deg_x = SCREEN_W / FOV_X
px_per_deg_y = SCREEN_H / FOV_Y
```

По умолчанию:

```text
SCREEN_W = 480
SCREEN_H = 320
FOV_X = 43.60°
FOV_Y = 33.40°
```

Камера `640x480` соотнесена с TFT `480x320` нормализацией полного кадра на
полный экран:

```text
display_x = camera_x * 480 / 640
display_y = camera_y * 320 / 480
```

Поэтому дефолтный `FOV_X/FOV_Y` дисплея равен дефолтному `HFOV/VFOV` камеры.

## 6. Формулы прошивки

Вход:

```text
target_azimuth_deg
target_elevation_deg
device_yaw_deg
device_roll_as_elevation_deg
```

Смещения:

```text
yaw_offset_deg = wrap180(target_azimuth_deg - device_yaw_deg)
elevation_offset_deg = target_elevation_deg - device_roll_as_elevation_deg
```

Экран:

```text
screen_x = CX + yaw_offset_deg * px_per_deg_x
screen_y = CY - elevation_offset_deg * px_per_deg_y
```

Знак yaw: положительный азимут цели уходит вправо на экране, то есть
`target_yaw - device_yaw`.

## 7. TCP-протокол

### Основной пакет цели

Формат оставлен совместимым со старым GUI:

```text
MSG:<message>;X:<elevation_offset_deg>;Y:<azimuth_offset_deg>\n
```

Где:

- `X` - смещение угла места от активного pitch-базиса, градусы;
- `Y` - смещение азимута от активного yaw-базиса, градусы;
- `message` - короткая подпись пакета, на TFT сейчас не выводится.

Пример:

```text
MSG:TARGET;X:4.5;Y:18.0
```

`X=0;Y=0` центрирует цель в активном базисе устройства. Стартовый активный базис
создается после быстрой zero-калибровки, а `CMD:CENTER` переносит его в
текущую ориентацию. Это основная
семантика центровки: нулевая цель от клиента должна оставлять квадрат в центре
после zero-калибровки, а повторная отправка одного и того же `MSG` не должна
накопительно сдвигать цель.
Положительный `Y` рисуется справа, положительный `X` рисуется выше центра.

### Служебные команды

Команды принимаются по TCP и UDP на порту `3333`.

```text
PING
CMD:CENTER
CFG:FOV_X:<deg>
CFG:FOV_Y:<deg>
CFG:TCP_POLL_MS:<ms>
CFG:BOX_SIZE:<px>
CFG:BOX_REFRESH_MS:<ms>
CFG:BOX_RENDER:<0|1>
CFG:BOX_DELTA_RENDER:<0|1>
MSGONLY:<text>
```

`CMD:CENTER` переносит активный базис цели в текущую ориентацию устройства и
центрирует маркер. После этого обычные `MSG` трактуются как смещения от нового
базиса.

`MSGONLY:<text>` принимается Arduino без изменения углов цели; на TFT строка
`Msg` сейчас не выводится.

`CFG:DEG_PER_PX:<value>` оставлен только как legacy-команда. Прошивка переводит
ее в:

```text
FOV_X = value * SCREEN_W
FOV_Y = value * SCREEN_H
```

Jetson control panel отправляет `PING` примерно раз в секунду без записи в лог.
Это удерживает TCP-соединение активным, потому что Arduino закрывает клиента по
`TCP_ALIVE_TIMEOUT_MS`, если долго не получает входящие строки.

## 8. Репозиторий после чистки

Оставлено:

- `src/main.cpp` - `setup()`, `loop()` и порядок работы модулей;
- `src/config.h` - пины, размеры экрана, таймауты и настройки по умолчанию;
- `src/app_state.*` - общие объекты Arduino и состояние приложения;
- `src/angle_utils.h` - угловая математика и clipping прямоугольников;
- `src/imu_zero.*` - IMU DMP, чтение FIFO, прогрев и zero-калибровка;
- `src/ui.*` - прицел, статусы и countdown;
- `src/marker.*` - отрисовка и очистка маркера цели;
- `src/net.*` - Wi-Fi AP, TCP/UDP и парсинг команд;
- `platformio.ini` - PlatformIO-профиль;
- `lib/ILI9488/` - локальная библиотека дисплея;
- `tools/windows_tcp_gui.py` - Windows GUI;
- `tools/jetson_tcp_gui.py` - Linux/Jetson GUI;
- `tools/jetson_wifi_visualize.py` - Jetson Linux control panel без графиков и без обратной телеметрии Arduino;
- `tools/yolo11n.pt` и `tools/quadron_1280.onnx` - опциональные модели для GUI;
- `requirements-jetson-viz.txt` - зависимости Jetson control panel;
- `README.md` и `task.md`.

Удалено:

- `tools/deepstream_quadron/`;
- DeepStream custom parser и конфиги;
- TensorRT engine-конфигурация;
- `trilateration_gui.py`, потому что UWB пока не используется;
- старый Processing serial animation sketch.

Игнорируется:

- `.pio/`;
- `.venv/`;
- `__pycache__/`;
- `runs/`;
- локальные `*.engine`, `*.plan`, `*.so`;
- локальные веса `yolo_quadro_weights/`.

## 9. Сборка и запуск

Сборка:

```bash
pio run
```

Прошивка:

```bash
pio run -t upload
```

Монитор:

```bash
pio device monitor -b 115200
```

Windows GUI:

```bash
python tools/windows_tcp_gui.py
```

Запускается из Windows PowerShell; для подключения к AP Arduino использует
`netsh`. Остальной интерфейс совпадает с Jetson TCP GUI.

Linux/Jetson GUI:

```bash
python3 tools/jetson_tcp_gui.py
```

Jetson Wi-Fi control panel:

```bash
pip install -r requirements-jetson-viz.txt
python3 tools/jetson_wifi_visualize.py
```

Открыть:

```text
http://127.0.0.1:8050
```

Возможности:

- подключение к Arduino `192.168.4.1:3333` с выбором транспорта `UDP/TCP`;
- задание цели углами `azimuth/elevation/range`;
- расчет контрольных координат цели `X/Y/Z`;
- отправка текущей цели на Arduino в формате `MSG:JETSON;X:<elevation_offset>;Y:<azimuth_offset>`;
- круговое движение цели для проверки отрисовки маркера на TFT;
- захват цели через камеру: YOLO, отдельный контрастный трекер или ORB
  (`Oriented FAST and Rotated BRIEF`) с настройками фильтров, стабилизации,
  сглаживания и удержания;
- отправка `MSGONLY:<text>` на Arduino без изменения цели;
- изменение FOV, размера квадрата, частот TCP/отрисовки и флагов рендера;
- отображение локального preview цели без Wi-Fi телеметрии.
- IMU serial log `IMU,...` примерно 10 раз в секунду и Python-график
  `tools/plot_imu_com4.py` для COM4/CSV.

## 10. Требования к поведению

- После старта устройство должно дождаться IMU и быстро выполнить нулевую
  калибровку только после короткого непрерывного стабильного окна IMU.
- До завершения нулевой калибровки рисуется прицел, countdown и центрированный
  квадрат цели.
- После нулевой калибровки стартовая цель должна быть зафиксирована в
  направлении `pitch=0/yaw=0`, чтобы движение IMU смещало квадрат без первой
  команды клиента.
- Без активного клиента устройство должно показывать `NET WAIT`.
- При подключении TCP-клиента статус должен стать `TCP OK`; при получении UDP
  пакетов или `PING` статус должен стать `UDP OK`.
- Пакет `MSG` должен обновлять цель только в RAM.
- Jetson-кнопка Center Arduino должна отправлять `CMD:CENTER`, чтобы
  центрировать цель и создать новый активный базис на текущей ориентации.
- EEPROM должен быть выключен: цель не должна восстанавливаться после reset.
- Yaw-смещение должно считаться как `target_yaw - device_yaw`, чтобы положительный азимут рисовался справа.
- Цель выше текущего roll-derived elevation должна двигать маркер вверх.
- `MSG` с `X=0;Y=0` должен центрировать цель в активном базисе устройства.
- Изменение `FOV_X/FOV_Y` должно менять масштаб маркера без перепрошивки.
- Маркер должен оставаться частично видимым у края экрана.
- Jetson control panel должен иметь предполагаемую дальность цели `20 м` по умолчанию.

## 11. Будущие задачи

- Перейти с `String` в TCP parser на фиксированный `char[]` буфер.
- Вынести Wi-Fi credentials и тайминги в отдельный профиль конфигурации.
- Добавить watchdog/recover для Wi-Fi.
- Добавить sequence number и диагностический ACK-режим.
- После появления UWB-части перенести из `berdanka-vizualize` расчет
  `target - device_position` и сравнение ошибок `no_uwb` / `uwb`.
